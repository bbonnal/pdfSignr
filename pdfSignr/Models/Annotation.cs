...
public partial class TextAnnotation : Annotation
{
    ...
    internal Typeface MeasureTypeface() => TypefaceCache.GetOrAdd(FontFamily,
        ff => new Typeface(Measurer?.GetMeasureFontFamily(ff) ?? new FontFamily("Sans Serif")));

    private double ComputeFontSize()
    {
        if (_heightPt <= 0) return 12;
        if (string.IsNullOrEmpty(FontFamily) || Measurer == null) return _heightPt;
        const double refSize = 72.0;
        double dpiScale = PdfConstants.DpiScale;
        var ft = new FormattedText("Xg", CultureInfo.InvariantCulture,
            FlowDirection.LeftToRight, MeasureTypeface(), refSize * dpiScale, Brushes.Black);
